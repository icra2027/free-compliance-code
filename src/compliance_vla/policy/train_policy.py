#!/usr/bin/env python3
"""Unified launcher for the force/compliance baselines that share a dataset
and evaluation harness:
  ...
  B3  pretrained VLA + force input,   HYBRID force-position output
                                       (ForceVLA2/Force Policy style)
  B4  ACT/Bi-ACT FROM SCRATCH,        compliance output  (H4 control)
...
exactly what "matched data and backbone" requires for B0/B2/B3/B5;
...
One invocation trains one policy/seed. Use `--smoke-test` to verify a
policy/seed combination trains without NaNs before committing GPU time to the
real run.

Usage:
    # Correctness check only, a few real steps, ~1-2 min:
    python -m compliance_vla.policy.train_policy --policy b0 --seed 0 --smoke-test
    python -m compliance_vla.policy.train_policy --policy b2 --seed 0 --smoke-test
    python -m compliance_vla.policy.train_policy --policy b3 --seed 0 --smoke-test
    python -m compliance_vla.policy.train_policy --policy b4 --seed 0 --smoke-test
    python -m compliance_vla.policy.train_policy --policy b5 --seed 0 --smoke-test

    # One real run (one per policy x seed):
...
from compliance_vla.policy._paths import RELEASE_ROOT, ensure_on_sys_path  # noqa: E402
...
STATE_DIM = 20  # q(7) + qdot(7) + x_f(6)
IMAGE_SHAPE = (3, 224, 224)
TRAIN_SESSIONS = ["demo4"]  # session-level split, data_extraction/build_dataset_splits.py -- the
...
# session for training instead, e.g. for the language-grounding retraining ablation, which
# retrains on the full 92-episode dataset rather than demo4's 32. Doing so means there is no held-out val/test left for that
# run -- fine for that ablation (it's scored by src/compliance_vla/policy/diagnose_language_grounding.py, not
...
def build_config(args):
    ...
    normalization_mapping = {
        ...
        "STATE": NormalizationMode.IDENTITY,  # no dataset-wide normalization stats are computed
    ...
    common = dict(
        ...
        # discrimination. One live hypothesis:
...
def main():
    ...
    p.add_argument("--unfreeze-lm", action="store_true",
                    help="set config.train_expert_only=False so the SmolVLM text tower is trained too, not just "
                         "the action expert -- the language-grounding retraining ablation. Vision encoder stays frozen unless --unfreeze-vision "
    ...
    p.add_argument("--unfreeze-vision", action="store_true",
                    help="set config.freeze_vision_encoder=False so the SigLIP vision tower is trained too -- "
                         ...
                         "src/compliance_vla/policy/diagnose_language_grounding.py, since the failure may be that the frozen "
    ...
    p.add_argument("--overfit-n-per-referent", type=int, default=None,
                    help="colour-balanced overfit sanity check: cap the dataset to this many episodes per colour referent per "
                         "session, picked the same way src/compliance_vla/policy/diagnose_language_grounding.py's "
                         ...
                         "convergence and be evaluated with src/compliance_vla/policy/diagnose_language_grounding.py after.")
    ...
    p.add_argument("--referents", nargs="+", default=None, choices=("red", "blue", "green", "black"),
                    help="2026-09-01 2-colour descope: restrict "
    ...
    p.add_argument("--lora", action="store_true",
                    help="wrap the attention projection layers that actually see language (SmolVLM's own "
                         ...
                         "of fully unfreezing them. The full-unfreeze "
                         ...
                         "the adapter structure, which is what folding the adapters into plain weights for "
                         "serving needs.")
    ...
    p.add_argument("--no-manner-calibration", action="store_true",
                    help="disable the per-operator log_k calibration (compliance_vla.policy.labels.calibrate_log_k, "
                         "fit by src/compliance_vla/policy/fit_manner_force_calibration.py) that's otherwise applied by "
    ...
    if args.lora and args.policy == "b4":
        ...
        if not lora_target_modules:
            raise ValueError(
                ...
                "have changed since this filter was written (src/compliance_vla/policy/train_policy.py's --lora block)"
            )
    ...
    if args.referents is not None:
        ...
        from compliance_vla.policy.diagnose_language_grounding import referent_from_task
    ...
    if args.overfit_n_per_referent is not None:
        ...
        from compliance_vla.policy.diagnose_language_grounding import _resolve_session_dir, pick_balanced_episodes
        ...
        for batch in loader:
            ...
            if args.checkpoint_dir and not args.smoke_test and step % args.checkpoint_every == 0:
                ...
                if args.lora:
                    ...
                    # in". Folding the adapters into plain weights, so serve_policy.py can load the
                    # checkpoint unmodified, needs exactly this.
                    ckpt["lora_config"] = {
                        "r": args.lora_r, "lora_alpha": args.lora_alpha, "lora_dropout": args.lora_dropout,
