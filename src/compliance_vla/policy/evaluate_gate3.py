#!/usr/bin/env python3
"""Evaluate the in-distribution pilot check: B5 >= B0 on the T1 pilot. If not,
debug the controller (is realized K tracking commanded K?) before touching the
model.

Consumes the per-rollout JSON logs scripts/run_pilot_rollout.py writes for
n=5 T1 in-distribution pilot rollouts per policy. These pilots are sanity
checks only -- they are NOT evaluation rollouts and are not reported. Because
n=5 is far below the real evaluation budget (n=20/cell), the comparison here
is a plain mean-ink-removal comparison, NOT a CI-based statistical test --
Holm-Bonferroni / Wilson CIs / bootstrap are reserved for the real evaluation
(E1), and using them here would falsely dress up a 5-rollout sanity check as a
real result. This is a documented design choice, the same pattern as
evaluate_gate1.py's (ii)/(iii) or the M8 task definition -- stated here rather
than silently decided.

On FAIL, this script does NOT touch the model. It prints the required next
diagnostic step -- re-verify realized-vs-commanded stiffness tracking
(external/fr3_bilateral_teleop/scripts/probe_variable_impedance_sinusoid.py,
already validated on real hardware) -- and stops there. Whether the controller or the model is
actually at fault is a judgment for whoever runs that diagnostic, not
something this script auto-decides.

Usage:
    python -m compliance_vla.policy.evaluate_gate3 --b0-logs 'reports/pilot_rollouts/b0_*.json' \\
                                      --b5-logs 'reports/pilot_rollouts/b5_*.json'
    python -m compliance_vla.policy.evaluate_gate3 --self-test   # synthetic rollout logs, no hardware needed
"""
import argparse
import glob
import json
...
DIAGNOSTIC_MESSAGE = """
PILOT CHECK: FAIL -- B5 did not beat B0 on the in-distribution T1 pilot.

Debug the controller (is realized K tracking commanded K?) before touching the
model. The controller is the likelier culprit.
...
Next step, in order -- do NOT retrain or otherwise modify the model yet:
  ...
  2. Compare the resulting commanded-vs-realized-stiffness + energy-tank
     figure against the controller-validation baseline
     ...
     (data_extraction/fit_frozen_contact_frame.py) is dropping a lot of the
...
def run_self_test() -> int:
    ...
    print("\ncompliance_vla.policy.evaluate_gate3 self-test: PASS")
    return 0
...
def main() -> int:
    ...
    p.add_argument("--b2-logs", help="optional, context only -- not part of the pilot-check decision")
    p.add_argument("--b3-logs", help="optional, context only -- not part of the pilot-check decision")
    p.add_argument("--b4-logs", help="optional, context only -- not part of the pilot-check decision")
    ...
    print("=== Pilot summary (sanity only -- NOT evaluation numbers, do not report) ===")
    ...
    print(f"\nPilot check decision: {result['decision']}")
    ...
    print(f"B5 mean ink-removal {result['b5_mean_ink_removal_pct']:.1f}% >= "
          ...
          f"(margin {result['margin_pct']:+.1f} pct pts).")
    return 0
