#!/usr/bin/env python3
"""pure-numpy helpers for scripts/run_pilot_rollout.py (external/fr3_bilateral_teleop)
-- everything the pilot-rollout harness needs that does NOT depend on ROS2/rclpy, split out
so it can be unit-tested in this venv (which has no rclpy, see smolvla_policy/README.md) rather than
...
1. Rotating a predicted stiffness vector from the auto-fit CONTACT frame (what B5/B3's
   ...
   the exact controller-integration question src/compliance_vla/policy/labels.py's own docstring
   already flags as out of scope for training; data_extraction/fit_frozen_contact_frame.py explains
   why a PILOT rollout resolves it with a fixed, offline-fit contact frame rather
...
2. Deciding, at each control-loop tick, which step of the last-predicted action chunk to
   command -- the policy replans at 10Hz and the controller runs at 1kHz, with cubic-
   spline interpolation on x_eq and a log-space stiffness rate limiter (already built,
   external/fr3_bilateral_teleop's variable_impedance_controllers integration) doing the actual
   ...
   hardware, see the commanded-vs-realized-stiffness figure) do the rest; this is
...
def _self_test():
    ...
    print("src/compliance_vla/policy/controller_frame_utils.py self-test: PASS")
