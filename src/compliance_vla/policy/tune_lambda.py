#!/usr/bin/env python3
"""Tune lambda (the compliance/log_k loss weight) on the validation split.
...
session (demo1 -- the frozen session-level split) and record the mean
...
here is what the real, full B5 x 3-seed runs should be launched with via
train_policy.py, not this script's own short-trained weights.

Selection rule (a documented choice, not a formula fixed by the method, same
...
Usage:
    ...
    python -m compliance_vla.policy.tune_lambda --lambdas 0.1 1.0 10.0 --tune-steps 500 --smoke-episodes 6  # quick check
"""
...
from compliance_vla.policy._paths import RELEASE_ROOT  # noqa: E402
...
from compliance_vla.policy.train_policy import STATE_DIM, IMAGE_SHAPE, set_seed  # noqa: E402

TRAIN_SESSION = "demo4"
VAL_SESSION = "demo1"  # session-level split: train=demo4, val=demo1, test=demo3
...
def main():
    ...
    print(f"[tune_lambda] use the winner for the real runs: "
          f"python -m compliance_vla.policy.train_policy --policy b5 --lam {best['lambda']} ...")
