...
Loads one checkpoint written by scripts/train_policy.py's --checkpoint-dir
...
wherever the checkpoint + venv are.
    ...
    python3 scripts/serve_policy.py --checkpoint checkpoints/b5_seed0/b5_seed0_step30000.pt
    python3 scripts/serve_policy.py --checkpoint checkpoints/b5_seed0/b5_seed0_step30000.pt --chunk-size 16

Needs fastapi, uvicorn, json-numpy on top of the usual venv (scripts/README.md):
...
`resp.json()` (see scripts/client_example.py, and the note by the import
below on why this server deliberately does not call `json_numpy.patch()`):

    {
      ...
      "state":      <float32 array, (20,)>,        # [q(7), qdot(7), x_f(6)] -- STATE_DIM, scripts/train_policy.py
...
config used (32 for the Day-12/14 baselines). On error:
...
from scripts/README.md, which is a different machine entirely):

    ssh -N -L 8000:localhost:8000 <user>@<server-host>

Then on the robot machine, POST to http://localhost:8000/act as if the
server were local -- see scripts/client_example.py for a minimal client.
"""
...
def main():
    ...
    p.add_argument("--checkpoint", required=True,
                   help="path to a .pt file written by scripts/train_policy.py's --checkpoint-dir")
