"""Day 10: "Build the LeRobot dataset with session-level splits. Record
...
analysis benefit (none of this project's Day 9/10 deliverables touch pixels,
...
nowhere near the proposal's eventual target of dozens of sessions -- flagged
...
rather than random:
  - test  = demo3 (operator B) -- held out entirely. This also gives the
    proposal's own §5 "cross-operator held-out test" (train on A, evaluate on
    B) for free, rather than wasting the one non-A session on an arbitrary
    train/val role.
