#!/usr/bin/env python3
"""Day 14: T1's primary metric (proposal §5, M2): per-mark pixel-difference ink-removal
...
... as a percentage. Continuous, automatic, no human labelling." (§5, T1). This reuses this
...
proposal-specified formula" pattern (evaluate_gate1.py's (ii)/(iii), offline_stiffness_benchmark.py's
...
Usage:
    python scripts/score_ink_removal.py --before before.png --after after.png \\
        --rois red:120,80,60,60 blue:300,80,60,60 green:120,260,60,60 black:300,260,60,60
    python scripts/score_ink_removal.py --self-test   # synthetic images, no camera needed
"""
import argparse
import json
import sys
...
def crop_roi(img: np.ndarray, roi):
    """roi: (x, y, w, h) in pixels, top-left origin -- matches proposal §5's "fixed ROI
    around each of the 4 marks" wording and the grid layout it specifies (corners + centre)."""
    x, y, w, h = roi
...
def score_mark(before_roi: np.ndarray, after_roi: np.ndarray, diff_threshold: float = 25.0) -> dict:
    ...
    `diff_threshold`, as a percentage -- the proposal's literal metric ("pixel-difference
...
def run_self_test() -> int:
    ...
    print("scripts/score_ink_removal.py self-test: PASS")
    return 0
