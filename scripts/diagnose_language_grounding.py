...
running scripts/serve_policy.py.

    python3 scripts/diagnose_language_grounding.py --session demo1 --episode 3
...
# 2-colour descope (2026-09-01 decision gate, see language_grounding_issue_handoff.md):
...
def get_action_chunk(server_url, scene_rgb, wrist_rgb, state, task, force_history=None, seed=None):
    ...
    _resolve_session_dir/pick_balanced_episodes (as scripts/train_policy.py's
...
def main():
    ...
    tool_offset = np.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), "tool_offset.npy"))
