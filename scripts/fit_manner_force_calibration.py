...
index mapping fix already established and tested there -- see
language_grounding_issue_handoff.md's "MAJOR CORRECTION") rather than re-deriving that
mapping here.

Usage:
    python3 scripts/fit_manner_force_calibration.py
Writes scripts/manner_force_calibration.json (loaded by compliance_vla.policy.labels.load_manner_calibration).
"""
...
OUT_PATH = os.path.join(SCRIPT_DIR, "manner_force_calibration.json")
...
def main():
    ...
    out = {
        "description": "Per-operator log_k calibration for cross-operator manner grounding "
                        "-- see scripts/fit_manner_force_calibration.py docstring.",
