...
src/compliance_vla/policy/serve_policy.py's /act endpoint) on the real Franka follower arm.

Talks to the same server contract as compliance-vla/src/compliance_vla/policy/client_example.py:
...
because the whole point of the method's identifiability argument is that x_eq is
...
# compliance-vla/data_extraction/tool_offset.npy works under both.
def _find_bookish_scripts_dir(start_dir, max_levels=10):
    candidate = os.path.abspath(start_dir)
    for _ in range(max_levels):
        scripts_dir = os.path.join(candidate, "src", "compliance-vla", "data_extraction")
    ...
    raise FileNotFoundError(
        "Could not locate compliance-vla/data_extraction/tool_offset.npy by searching "
...
# Franka flange -> wiper-tip offset, fit by compliance-vla/data_extraction/
...
# Controller-realizable stiffness range, used to clip
...
# Temporal ensembling across overlapping action chunks (ACT-style temporal
# ensembling across overlapping chunks) -- NOT previously implemented in
...
# hypothesis for why b0 needed steps_to_execute>8 while b5 was fine at 3
# (investigation notes, 2026-09-10).
#
...
# compliance-vla/data_extraction/dataset_io.py's parse_referent_from_task /
# parse_manner_from_task enums exactly
...
# already expects them. Deliberately excludes the held-out OOD manner
...
class SmolVLADeployment(Node):
    def __init__(
        ...
        # rotated from the training distribution,
        ...
        # measured directly (2026-09-07/08;
        # compliance-vla/src/compliance_vla/policy/check_state_convention_sensitivity.py). Fixed by
        ...
        # use_scene_cv2_capture fallback); the kernel's
        ...
        # 2026-09-07/08, just via a different path.
...
def main():
    rclpy.init()
    # Instruction (format: "wipe the <colour> mark <manner>") is now picked per-episode from a GUI dialog --
