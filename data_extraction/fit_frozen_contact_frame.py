#!/usr/bin/env python3
"""Fit ONE frozen T1 contact frame (board-normal rotation, base frame) for the
...
flags "Rotating predicted K back to base frame for the low-level controller is a
...
justified because the board is physically rigid and both task setups were frozen before
data collection (any later change would invalidate earlier sessions), so its normal does
not change between episodes or sessions. (b) is what this script does: it is strictly safer for
a PILOT rollout (sanity-only, n=5, not reported) than adding a live re-fit
...
than any new numerical logic. A live per-rollout re-fit is a reasonable upgrade if
...
demo4 -- train_policy.py's TRAIN_SESSIONS), then calls fit_contact_frame() ONCE on the pooled set.
...
Usage:
    python data_extraction/fit_frozen_contact_frame.py                       # pools demo4 (default)
    python data_extraction/fit_frozen_contact_frame.py --sessions demo1 demo4
    python data_extraction/fit_frozen_contact_frame.py --self-test           # synthetic plane, no dataset needed
"""
import argparse
import json
import os
...
DEFAULT_SESSIONS = ["demo4"]  # matches src/compliance_vla/policy/train_policy.py's TRAIN_SESSIONS -- the frozen train split
...
def run_self_test() -> int:
    """Synthetic plane, tilted ~20deg from vertical (matching the real board
